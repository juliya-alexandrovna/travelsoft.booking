...
<form enctype="multipart/form-data" action="<?= $APPLICATION->GetCurPageParam() ?>" method="POST" id="<?= $formName ?>" name="<?= $formName ?>">
    ...
    <div class="row">
                ...
                <div class="col-md-<?= $col ?> col-lg-<?= $col ?> col-sm-<?= $col ?>">
                    ...
                    foreach ($arFormData as $fieldName => $arFieldData):
                        ...
                        <? if ($arFieldData['USER_TYPE']['USER_TYPE_ID'] == 'file'): ?>
                            <div class="hr-form-group-<?= $fgindex; ?>"></div>
                            <div class="<? if ($fieldName === 'UF_FILE') echo 'parent' ?> col-md-6 col-lg-6 col-sm-6 form-group-<?= $fgindex; ?>">

                                <label <? if ($fieldName === 'UF_FILE') echo 'class="' . $fieldsBlocksHiddenClass["UF_FILE"] . '"' ?> for="<?= $fieldName ?>">
                                ...
                                <div <? if ($fieldName === 'UF_FILE') echo 'class="' . $fieldsBlocksHiddenClass["UF_FILE"] . '"' ?> id="old-file-area-<?= $key ?>">
                                    <?
                                ...
                                <input <? if ($fieldName === 'UF_FILE') echo 'class="' . $fieldsBlocksHiddenClass["UF_FILE"] . '"' ?> type="file" name="TOURISTS[<?= $key ?>][<?= $fieldName ?>]">
                            </div>
                            ...
                            <div class="col-md-6 col-lg-6 col-sm-6">
                                <div class="checkbox">
                                    ...
                                    <label for="<?= $fieldName ?>">
                                        ...
                                        if ($fieldName == 'UF_NEED_VISA') {

                                            $fieldsBlocksHiddenClass["UF_FILE"] = $arFieldData['VALUE'] == 1 ? '' : 'hidden';

                                            $onclick = 'onclick="toggleLinkedField(\'[' . $key . '][UF_FILE]\')"';
                                        } elseif ($fieldName == 'UF_NEED_INSUR') {

                                            $fieldsBlocksHiddenClass["UF_PASS_PERNUM"] = $arFieldData['VALUE'] == 1 ? '' : 'hidden';

                                            $onclick = 'onclick="toggleLinkedField(\'[' . $key . '][UF_PASS_PERNUM]\')"';
                                        }
                                        
                                        ?>

                                        <input type="hidden" name="TOURISTS[<?= $key ?>][<?= $fieldName ?>]" value="0">
                                        <input <?= $onclick ?> <? if ($arFieldData['VALUE'] == 1) : ?>checked=""<? endif ?> type="checkbox" name="TOURISTS[<?= $key ?>][<?= $fieldName ?>]" value="1"> <b><?= $arFieldData['EDIT_FORM_LABEL'] ?></b><? if ($arFieldData['required']): ?><span class="starrequired">*</span><? endif ?> <? Utils::showError((array) $arResult['ERRORS'][$key], 'WRONG_' . $fieldName) ?>
        ...
        <? else: ?>
                            <div class="hr-form-group-<?= $fgindex; ?>"></div>
                            <div class="<? if ($fieldName === 'UF_PASS_PERNUM') echo 'parent' ?> col-md-6 col-lg-6 col-sm-6 form-group-<?= $fgindex; ?>">

                                <label <? if ($fieldName === 'UF_PASS_PERNUM') echo 'class="' . $fieldsBlocksHiddenClass["UF_PASS_PERNUM"] . '"' ?> for="<?= $fieldName ?>">
            ...
            <? else: ?>
                                    <input class="<? if ($fieldName === 'UF_PASS_PERNUM') echo $fieldsBlocksHiddenClass["UF_PASS_PERNUM"] ?> form-control" type="text" name="TOURISTS[<?= $key ?>][<?= $fieldName ?>]" value="<?= $arFieldData['VALUE'] ?>">
                                    <?
...
<? if (!empty($arResult['TOURISTS'])) { ?>
             ...
             * @returns {undefined}
             */
            window.toggleLinkedField = function (partName) {

                var field = BX.findChild(BX('<?= $formName ?>'), {attribute: {name: "TOURISTS" + partName}}, true);
                var parent = BX.findParent(field, {className: 'parent'});
                var children = BX.findChildren(parent);

                children.forEach(function (el) {
                    if (BX.hasClass(el, 'hidden')) {
                        BX.removeClass(el, 'hidden');
                    } else {
